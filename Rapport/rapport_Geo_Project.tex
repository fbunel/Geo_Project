...
\begin{equation}
 S = 4 \pi R^2 \sigma (T_{neb}^4 - (TT_0)^4)  \quad \textrm{à la surface uniquement}
\end{equation}
...
Exemple : on part du cas $T > T_{fus,met}$ , $ \phi_{met} < 1$ pour atteindre $\phi_{met} = 1$. Calculons la température finale : 

\begin{equation}
T_{f} =  (T_{i} - T_{fus,met} ) (1-\phi_{met}) \frac{\phi L_{met}}{C_{p,met} T_{fus,met}}
\end{equation}


\subsubsection{Équations discrétisées}

On pose $c = \frac{\rho C_p T_0}{\tau_{1/2}}$ 

On a l'équation matricielle suivante :
\begin{equation}
MT^{t+1} = T^t + \Delta t( P + S )
\end{equation}
