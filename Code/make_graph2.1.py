...
def graphes_T_moy():
    ...
    for i,T in enumerate([T1,T2]):
        axes[i].plot(T[0],T[4],lw=2,label=r'$\beta = 0$')
        axes[i].plot(T[0],T[5],lw=2,label=r'$\beta = 1$')
        axes[i].plot(T[0],T[6],lw=2,label=r'$\beta = 2$')

    ax1.set_xticks([0,0.5,1])
    ax1.set_xticklabels([0,0.5,1])
    ax1.set_ylabel(r'T moyenne (en K)')
    ax1.set_xlabel(r't (en My)')
    ax2.set_xlabel(r't (en My)')
    ax2.set_xlim(0,5)
    ax2.set_yticklabels([],visible=False)
    ax2.legend(loc='best')
    plt.tight_layout()

    plt.savefig("graph_sim2_fig2_4_imp_{}.pdf".format(datetime.now().strftime('%Y%m%d-%H%M%S')),bbox_inches='tight')
    ...
    T5 = np.array(T5)
    t1 = np.where(T5[:,0]<=1)
    ax1.plot(T5[t1,0],T[t1,1],lw=2,label=r'$\beta = \empyset$')
    ax2.plot(T5[:,0],T[:,1],lw=2,label=r'$\beta = \empyset$')
    ax1.set_xticks([0,0.5,1])
    ax1.set_xticklabels([0,0.5,1])
    ax1.set_ylabel(r'T (en K)')
    ax1.set_xlabel(r't (en My)')
    ax2.set_xlabel(r't (en My)')
    ax2.set_xlim(0,5)
    ax2.set_yticklabels([],visible=False)
